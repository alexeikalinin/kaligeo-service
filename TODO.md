...
> Обновлён: 2026-06-23 (после полной проверки прод-окружения). 8 платформ live-протестированы и работают (`/api/admin/platform-test`): ChatGPT, Claude, Gemini, Perplexity, DeepSeek, YandexGPT, Alisa, Grok. GigaChat — не настроен (нет аккаунта).

---

## 🔴 P0 — Рассинхрон git/деплой (сделать первым)

### 0a. Запушить последний коммит в GitHub
- `3a0c4c4` (фикс хардкода `app.kaligeo.ru` в URL отчёта) задеплоен на Vercel вручную, но **не запушен** в `origin/main` (`ahead 1`)
- Риск: следующий git-based деплой или `vercel --prod` с чистого клона откатит прод на старую версию с багом
- Действие: `git push origin main`

### 0b. Залить обновлённые лендинги на hoster.by
- Локально есть несохранённые правки SEO (`landing/ru/index.html`, `landing/by/index.html`, `.claude/skills/seo-landing.md`) — новые гео-ключевики (Москва, СПб, Краснодар и т.д.), не закоммичены и не залиты на хостинг
- Действие: проверить дифф → закоммитить → вручную залить оба файла через FTP/панель hoster.by → переиндексация в GSC/Я.Вебмастер

### 0c. Убрать случайные файлы из репозитория
- `qa-*.png` (6 файлов) и `app/🔍 Search — Аудит поисковых запросов 2026-05.pdf` — debug-скриншоты и случайно сохранённый PDF в `app/`, не должны быть в репозитории
- Действие: удалить или вынести за пределы репо
...
## ✅ Закрыто с прошлой проверки

### 1. Юридические документы — готово
- `/legal/offer`, `/legal/privacy`, `/legal/returns`, `/legal/payment` — все 4 страницы живые на проде (200 OK), плюс статические `privacy.html` для hoster.by (ru+by)
...
### 6. ~~Мониторинг новых платформ~~ — проверено, работает
- Grok и DeepSeek подтверждены live-тестом 2026-06-23, оба отвечают (`status: ok`)
