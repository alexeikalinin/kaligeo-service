...
function page(title: string, body: string, market: Market = "ru") {
...
export async function GET(req: NextRequest) {
  ...
  const job = await prisma.auditJob.findUnique({
    where: { id: jobId },
    select: { clientEmail: true, market: true },
  })
  ...
  return page(
    "Вы отписались",
    "Мы больше не будем присылать вам письма по аудитам и мониторингу AI-видимости.",
    (job.market as Market) ?? "ru"
  )
}
