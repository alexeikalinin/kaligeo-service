...
export interface GrowthPlan {
  ...
  contentCalendar: ContentTask[] // контент-план по неделям (1-12)
}
...
export async function runGrowthPlanAgent(input: GrowthPlanInput): Promise<GrowthPlan> {
  ...
  const prompt = `Ты — старший консультант по AI-видимости брендов. Твоя задача — создать детальный план внедрения и роста, который команда клиента сможет выполнить самостоятельно без дополнительных консультаций.
...
{
  "strategy": "Общая стратегия в 2-3 предложениях — что именно меняем и почему это поднимет score",
  ...
  "contentCalendar": [
    { "week": 1, "theme": "Тема контента недели", "tasks": ["Задача 1", "Задача 2"] },
    { "week": 2, "theme": "...", "tasks": [...] }
  ]
}
...
- 30d: 4-5 задач, фокус на технических быстрых исправлениях (Schema, meta, FAQ)
- 60d: 3-4 задачи, фокус на контенте и авторитете (публикации, кейсы, упоминания)
- 90d: 2-3 задачи, фокус на стратегическом позиционировании (партнёрства, отраслевые порталы)
- steps: минимум 3 конкретных шага для каждого пункта, не абстракции
- contentCalendar: 8 недель, реалистичная нагрузка (1-3 задачи в неделю)
